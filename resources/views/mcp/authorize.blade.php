...
<div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        <!-- Card Container -->
        <div class="rounded-lg border bg-surface-container-low text-on-surface shadow-sm">
            <!-- Header -->
            ...
            <div class="p-6 pt-0 space-y-4">
                <!-- User Info -->
                <div class="rounded-lg border p-4 bg-surface-container/50">
                    <p class="text-sm text-on-surface-variant mb-2">Logged in as:</p>
                    <p class="font-medium">{{ $user->display_name ?: $user->user_login }}</p>
            ...
            <div class="flex items-center p-6 pt-0 gap-3">
                ...
                <form method="POST" action="{{ route('passport.authorizations.deny') }}" class="flex-1">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-surface transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-outline-variant bg-surface hover:bg-surface-container-high hover:text-on-surface h-10 px-4 py-2 w-full">
                ...
                <form method="POST" action="{{ route('passport.authorizations.approve') }}" class="flex-1" id="authorizeForm">
                    ...
                    <input type="hidden" name="client_id" value="{{ $client->id }}">
                    <input type="hidden" name="auth_token" value="{{ $authToken }}">
                    <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-surface transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-[var(--on-primary)] hover:bg-primary/90 h-10 px-4 py-2 w-full" id="authorizeButton">
